...
class WorkspaceFileAssetEndpoint(BaseAPIView):
    ...
    def get_entity_id_field(self, entity_type, entity_id):
        ...
        # Mermaid diagrams are scoped to a project for access control
        if entity_type == FileAsset.EntityTypeContext.MERMAID_DIAGRAM:
            return {"project_id": entity_id}
        return {}
    ...
    def post(self, request, slug):
        ...
        # Check if the file type is allowed
        is_mermaid = entity_type == FileAsset.EntityTypeContext.MERMAID_DIAGRAM
        allowed_types = [
            "image/jpeg",
            "image/png",
            "image/webp",
        ...
        if type not in allowed_types or (type == "image/svg+xml" and not is_mermaid):
            return Response(
                {
                    "error": "Invalid file type. Only JPEG, PNG, WebP, JPG, GIF and SVG (for Mermaid) files are allowed.",
                    "status": False,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )

        # Mermaid diagrams must be bound to a project for access control
        if is_mermaid and not entity_identifier:
            return Response(
                {"error": "Mermaid diagrams require a project_id as entity_identifier.", "status": False},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        # Persist a source hash so the client can detect stale cached images
        attributes = {"name": name, "type": type, "size": size_limit}
        if is_mermaid:
            attributes["mermaid_source_hash"] = request.data.get("mermaid_source_hash")
    ...
    def get(self, request, slug, asset_id):
        ...
        # Get the presigned URL
        storage = S3Storage(request=request)
        # Mermaid diagrams are generated by our own renderer and are safe to display inline
        disposition = (
            "inline"
            if asset.entity_type == FileAsset.EntityTypeContext.MERMAID_DIAGRAM
            else "attachment"
        )
        # Generate a presigned URL to share an S3 object
